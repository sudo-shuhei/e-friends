...
RSpec.describe Request, type: :model do
  ...
  it "is invalid without a post_id" do
    ...
    request.valid?
    expect(request.errors[:post_id]).to include("を入力してください")
  end
  it "is invalid without a user_id" do
    request = Request.new(user_id: "")
    request.valid?
    expect(request.errors[:user_id]).to include("を入力してください")
  end
  it "is invalid without a post_user_id" do
    request = Request.new(post_user_id: "")
    request.valid?
    expect(request.errors[:post_user_id]).to include("を入力してください")
  end
  it "is invalid with a comment longer than 300" do
    request = Request.new(comment: "a"*400)
    request.valid?
    expect(request.errors[:comment]).to include("は300文字以内で入力してください")
  end
end
